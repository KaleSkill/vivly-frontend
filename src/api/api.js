...
export const userApi = {
    ...
    shiprocket: {
        ...
        trackOrder: (orderId) => {
            return api.get(`/shiprocket/track/${orderId}`);
        },

        // Track order by AWB code
        trackOrderByAWB: (awbCode) => {
            return api.get(`/shiprocket/track/awb/${awbCode}`);
        },

        // Get shipment details by shipment ID
        getShipmentDetails: (shipmentId) => {
            return api.get(`/shiprocket/shipment/${shipmentId}`);
        },

        // Get all shipments with pagination
        getAllShipments: (page = 1, perPage = 10) => {
            return api.get(`/shiprocket/shipments?page=${page}&per_page=${perPage}`);
        },

        // ==================== COURIER SERVICEABILITY ====================
        
        // Get courier serviceability
        getCourierServiceability: (pickupPincode, deliveryPincode, weight, codAmount = 0) => {
            return api.get(`/shiprocket/serviceability?pickup_pincode=${pickupPincode}&delivery_pincode=${deliveryPincode}&weight=${weight}&cod_amount=${codAmount}`);
        },

        // ==================== ORDER MANAGEMENT ====================
        
        // Create adhoc order
        createAdhocOrder: (orderData) => {
            return api.post(`/shiprocket/orders/create`, orderData);
        },

        // Assign AWB to order
        assignAWB: (shipmentId, courierId) => {
            return api.post(`/shiprocket/courier/assign/awb`, {
        ...
        generatePickup: (shipmentId) => {
            return api.post(`/shiprocket/courier/generate/pickup`, {
                shipment_id: shipmentId
            });
        },
        ...
        generateManifest: (shipmentIds) => {
            return api.post(`/shiprocket/manifests/generate`, {
                shipment_ids: shipmentIds
            });
        },

        // Print manifest
        printManifest: (manifestId) => {
            return api.get(`/shiprocket/manifests/print/${manifestId}`);
        },
        ...
        generateLabel: (shipmentId) => {
            return api.get(`/shiprocket/courier/generate/label/${shipmentId}`);
        },

        // Print invoice
        printInvoice: (orderId) => {
            return api.get(`/shiprocket/orders/print/invoice/${orderId}`);
        },
